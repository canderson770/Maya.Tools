...
def create_my_toolbox():
	...
	delete_history_button = cmds.button(parent=tab_1_column, label='Delete History', command='')
	freeze_transform_button = cmds.button(parent=tab_1_column, label='Freeze Transformations', command='')
	center_pivot_button = cmds.button(parent=tab_1_column, label='Center Pivot', command='')
	parent_scale_button = cmds.button(parent=tab_1_column, label='Parent and Scale Constrain', command='')
	
    #Poly Count
	cmds.separator(height=12, parent=tab_1_column)
	poly_count_button = cmds.button(parent=tab_1_column, label='Poly Count', command='')
	...
	display_axis_toggle = cmds.iconTextButton(style='iconOnly', image1='channelBoxUseManips.png', parent=orient_joint_row, command='')
